...
annual_salary = float(input('Enter starting rate: '))
monthly_salary = annual_salary / 12
steps = 0
portion_down_payment = 0.25 * total_cost

# Search 0 - 10000 as integer then convert to float
portion_saved = 5000
found = False
...
while not found:
    ...
    current_savings = 0
    steps += 1
    # TODO: Fix loop, not getting right answer as in the test case
    while current_savings < portion_down_payment - 100 and current_savings > portion_down_payment + 100:
    ...
    if months > 36:
        portion_saved -= int(portion_saved / 2)
    elif months < 36:
        portion_saved += int(portion_saved / 2)
    else:
        found = True
